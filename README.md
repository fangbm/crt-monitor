...
|---|---|
| `Tab` / `Shift+Tab` | 切换页面 |
| `E` | **布局编辑模式**：拖面板移动；任意边（左/右/上/下）与四角调整宽高（±16px 命中区）；松手吸附 2% 网格并写入 config.json |
| `C` | **卡片管理器**（编辑模式内）：每行 `ON`（反色高亮）/ `OFF`（暗色）切换当前页展示哪些卡片；底部 THEMES 区点击直接切换主题；新启用的卡片自动找空位；关闭的卡片记入页面 `hidden` 列表不会被自动补回；插件卡片带"插件"徽章 |
| `S` | 截图当前页面到 `图片\CRT-Monitor\` |
| `F11` | 全屏 ⇄ 窗口化 |
| `Esc` | 隐藏到托盘（彻底退出用托盘右键菜单 → 退出） |
| `R` | 重载前端 |
| `A` | 开机自启开关（HKCU Run，写回 config.json） |
| `T` | 循环切换主题（含 themes/ 目录主题，选择持久化） |
| `1` / `2` / `3` | 内建主题：磷光绿 / 琥珀 / 纸白 |

无操作 5 分钟自动进入屏保（暗色大时钟，防 CRT 灼屏），任意输入退出。托盘左键切换显隐，右键可导出/导入 config.json（换机/分享布局主题）。
